...
def img_to_ascii(img_path, width=80):
    img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
    if img is None:
        print("Error: source-prepped.png missing!")
        return []
    h, w = img.shape
    aspect = h / w
...
def create_svg(ascii_lines, output_path="avi-ascii.svg"):
    ...
    svg += '  <style>\n'
    svg += '    .term-text { font-family: ui-monospace, SFMono-Regular, SF Mono, Menlo, Consolas, monospace; font-weight: 600; }\n'
    svg += '  </style>\n'
    
    for i, line in enumerate(ascii_lines):
        # STRIP ALL XML CHARACTERS THAT BREAK GITHUB'S SECURITY ENGINE
        escaped_line = line.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace('"', "&quot;")
        y_pos = 25 + (i * line_height)
        
        svg += f'  <text x="10" y="{y_pos}" fill="{FILL_COLOR}" font-size="{font_size}" class="term-text">{escaped_line}</text>\n'
        
    svg += '</svg>'
    
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(svg)
    print(f"Success! Re-generated clean XML compliant layout: {output_path}")
